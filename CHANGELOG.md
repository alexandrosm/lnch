...
- GitHub Actions CI: PSScriptAnalyzer gate plus both stubbed suites and a remote-bootstrap smoke test on windows-latest. Tag pushes are packaged into a GitHub Release with SHA256SUMS by `.github/workflows/release.yml`.
...
- Initial release: create/reuse projects under `%OMP_PROJECTS_DIR%` (default `C:\projects`), `git init -b main`,
  ...
  intent metadata, agent registry (`agents.json`), project picker (fzf/numbered),
  Windows Terminal tab handoff, MIT license, engine test matrix.
