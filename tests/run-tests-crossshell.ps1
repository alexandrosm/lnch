...
try {
    ...
    $out = BashRun 'lnch --top --json'
    Check TOP-bash (($out -match '"Schema"\s*:\s*1') -and ($out -match '"Projects"\s*:')) $out
    ...
    $out = CmdRun ('"' + $cli + '" --top --json')
    Check TOP-cmd (($out -match '"Schema"\s*:\s*1') -and ($out -match '"Projects"\s*:')) $out
